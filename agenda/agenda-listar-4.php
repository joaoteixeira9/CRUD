<link rel="stylesheet" href="../css/agenda-listar.css">
        ...
        $data = date('Y-m-d');
        $sql = "SELECT * FROM agenda_4 " . $dataFiltro;
        $res = mysqli_query($conexao, $sql);

        $sqlData = "SELECT distinct data FROM agenda_4";
        ...
        while ($linha = mysqli_fetch_assoc($resData)) {
            echo " <a href='agenda-listar-4.php?dataFiltro=".$linha['data']."'><button class='btn'>" . date('d/m/Y', strtotime($linha['data'])) . "</button></a> " ;
        }
        echo "</div>";
        // Início da tabela
